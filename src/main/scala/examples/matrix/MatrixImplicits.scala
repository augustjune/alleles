...
class MatrixImplicits(source: String) {
  ...
  implicit val combinator: Join[Permutation] = Join.pure((perm1: Permutation, perm2: Permutation) => perm1.crossover(perm2))
  implicit val mutator: Modification[Permutation] = (perm: Permutation) => perm.mutate
