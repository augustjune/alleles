...
object ParentsOrOffspringProperties extends CrossoverProperties("ParentsOrOffspring props") {
  type Ind = String
  val implGen: Gen[ParentsOrOffspring] = choose[Double](0.0, 1.0).map(new ParentsOrOffspring(_))
  ...
  /**
    ...
    */
  val parentsBiasedGen: Gen[ParentsOrOffspring] = sized(n => choose(1.0, math.max(n, 1.0))).map(new ParentsOrOffspring(_))
  ...
  /**
    ...
    */
  val offspringBiasedGen: Gen[ParentsOrOffspring] = sized(n => choose(-n, 0.0)).map(new ParentsOrOffspring(_))
